...
export default function ScreenEditPanel() {
	...
	return (
		<div className="fixed top-0 right-0 h-full w-96 bg-slate-950 shadow-2xl z-50 flex flex-col border-l border-slate-800 overflow-hidden">
			{/* Header */}
				...
				{!screen ? (
					...
					<>
						...
						{(
							...
							return (
								<div key={field}>
									...
									<input
										className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-sm text-slate-200 focus:outline-none focus:border-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
										value={(val as string) ?? ""}
						...
						<div>
							...
							<span className="inline-block px-2 py-0.5 rounded text-xs font-semibold bg-slate-800 border border-slate-700 text-slate-300">
								{screen.screen_type}
							</span>
						</div>
								...
								<ul className="space-y-1">
									{menuItems.map(item => (
										<li
											...
											className="text-xs bg-amber-950/40 border border-amber-800/50 rounded px-2 py-1.5"
										>
											<span className="font-semibold text-amber-400">{item.option}.</span>{" "}
											<span className="text-slate-300">{item.display_name}</span>{" "}
											<span className="text-slate-500">→ {item.next_screen}</span>
										</li>
									))}
								</ul>
							</div>
							...
							<div>
								...
								<ul className="space-y-1">
									{routerOptions.map((opt, i) => (
										<li
											key={opt.id ?? i}
											className="text-xs bg-orange-950/40 border border-orange-800/50 rounded px-2 py-1.5"
										>
											<code className="font-mono text-orange-400">{opt.router_option}</code>{" "}
											<span className="text-slate-500">→ {opt.next_screen}</span>
										</li>
									))}
								</ul>
							</div>
			...
			{/* Footer */}
			{screen && (
				<div className="px-4 py-3 border-t border-slate-800 bg-slate-900 flex items-center gap-3">
					<button
						onClick={handleSave}
						disabled={saving}
						className="flex-1 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-sm font-medium rounded px-3 py-2 transition-colors"
					>
