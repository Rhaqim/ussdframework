    ...
    with_database(move |_manager| {
        ...
        async move {
            match result {
                ...
                Err(e) => HttpResponse::InternalServerError().body(format!("Error getting screen: {e}")),
            }
        }
    })
    .await
...
pub async fn get_multiple(query: web::Query<super::MultipleQuery>) -> impl Responder {
    with_database(move |_manager| {
        ...
        let result =
            <DatabaseManager as Database<ScreenModel>>::get_by_query_enum(_manager, query_enum);

        futures_util::future::Either::Left(async move {
            match result {
                Ok(screens) => HttpResponse::Ok().json(screens),
                Err(e) => HttpResponse::InternalServerError().body(format!("Error getting screens: {e}")),
            }
        })
    })
    .await
